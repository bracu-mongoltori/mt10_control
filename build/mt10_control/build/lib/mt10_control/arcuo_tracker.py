...
class ArucoTracker(Node):
    def __init__(self, max_lin_vel=45.0, max_ang_vel=5.0):
        ...
        self.ar_tag_state = self.create_subscription(
            Bool, "/ar_tag_state", self.ar_state, 10
        )

        self.camera = self.create_timer(0.01, self.camera_callback)
        self.ar_tag = self.create_timer(0.05, self.look_for_tag)
    ...
    def look_for_tag(self):
        if self.ar_state == False:
            ...
            lower_limit = center_x - 90
            upper_limit = center_x + 90

            if self.tag_detected == False:
                self.get_logger().info("No Aruco markers detected")
                self.get_logger().info("Searching for AR Tag")
                self.dir_msg = "right"
                message = self.right

            if self.tag_detected == True:
                self.get_logger().info("Aruco markers detected")
                if (
                    self.marker_center[0] < upper_limit
                    and self.marker_center[0] > lower_limit
                ):
                    ...
                    message = self.right if val < 0 else self.left
                    self.dir_msg = "right" if val < 0 else "left"

            self.rover.publish(message)
            self.get_logger().info(self.dir_msg)
        ...
        if ids is not None:
            for i in range(len(ids)):
                ...
                self.distance = self.calculate_distance(
                    self.marker_center, image_center, marker_width_pixels
                )
